...
## i/o functions
def print_run_stats(bam_file, loci, sample_id)
...
def print_reads(read_set, sample_id, locus, to='stdout', format='tsv', file_tag='')
  ...
  if to != 'stdout' and ! File.directory?(to)
    ...
    if file_tag.empty?
      #FileUtils.touch(to + "/#{sample_id}.#{amplicon}.reads.#{format}")
      out = open(to + "/#{sample_id}.#{amplicon}.reads.#{format}", 'w')
    else
      #FileUtils.touch(to + "/#{sample_id}.#{locus_str}.#{file_tag}.reads.#{format}")
      out = open(to + "/#{sample_id}.#{locus_str}.#{file_tag}.reads.#{format}", 'w')
    end
    out.write("")
  end
  if read_set.empty? 
  ...
  if format == 'tsv'
    line = "sample_id\tlocus\tamplicon\tread_idx\thaplotype_idx\tedit_dist\tlen\tcigar\tmismatch\tqual\tread\n"
    (to == 'stdout') ? (puts line) : (out.write(line))
    read_set.each{ |read| 
      if read[:mismatch_str] != nil
        line = "#{sample_id}\t#{read[:chrom]}:#{read[:pos]}\t#{amplicon}\t#{read[:read_idx]}\t#{read[:haplotype_idx]}\t#{read[:edit_dist]}\t#{read[:seq].length}\t" +
          "#{to_cigar_str(read[:cigar])}\t#{read[:mismatch_str]}\t#{read[:qual_str]}\t#{read[:seq]}\n"
      else
        line = "#{sample_id}\t#{read[:chrom]}\t#{read[:pos]}\t#{amplicon}\t#{read[:read_idx]}\t#{to_cigar_str(read[:cigar])}\t#{read[:seq].length}\t#{read[:seq]}\t#{read[:qual_str]}\n"
      end
      (to == 'stdout') ? (puts line) : (out.write(line))
    }
  end
...
def print_haplotypes(haplotype_set, sample_id, locus, to='stdout', include_read_idx=false, 
...
=end
  ...
  if to != 'stdout' and ! File.directory?(to)
    ...
    if file_tag.empty?
      out = open(to + "/#{sample_id}.#{amplicon}.haplotypes.#{format}", 'w')
    else
      out = open(to + "/#{sample_id}.#{amplicon}.#{file_tag}.haplotypes.#{format}", 'w')
    end
    out.write("")
  end
  # sort by coverage
  ...
  if format == 'tsv'
    if include_read_idx != false
      line = "sample_id\tlocus\tamplicon\tcoverage\tfreq\tlen\tread_idx\thaplotype_cluster_idx\thaplotype_idx\tpass_coverage\tpass_freq\tpass_len\thaplotype\n"
    else
      line = "sample_id\tlocus\tamplicon\tcoverage\tfreq\tlen\thaplotype_cluster_idx\thaplotype_idx\tpass_coverage\tpass_freq\tpass_len\thaplotype\n"
    end
    (to == 'stdout') ? (puts line) : (out.write(line))
  end
  if haplotype_set.empty?
    FileUtils.touch(to + "/#{sample_id}.#{amplicon}.haplotypes.#{format}")
    return
  end
  haplotype_set.each{ |haplotype|
    #haplotype_locus_str = locus_to_str([haplotype[:chrom], [haplotype[:start], haplotype[:end]]])
    if format == 'fasta'
      line = ">#{sample_id}__#{haplotype[:haplotype_locus]}__#{haplotype[:coverage]}__#{haplotype[:final_freq]}"+"\n"+haplotype[:haplotype]+'\n'
    elsif format == 'tsv'
      if ! with_extras
        if include_read_idx
          line = "#{sample_id}\t#{haplotype[:haplotype_locus]}\t#{amplicon}\t#{haplotype[:coverage]}\t#{haplotype[:final_freq].round(8)}\t#{haplotype[:haplotype_len]}\t" + 
        ...
        line = "#{sample_id}\t#{haplotype_locus_str}\t#{amplicon}\t#{haplotype[:coverage]}\t#{to_freq(haplotype[:coverage],total)}\t"+\
...
def print_alleles(allele_counts, locus, sample_id, to='stdout')
  locus_str = locus_to_str(locus)
  if to != 'stdout' and ! File.directory?(to)
    raise(Exception, "Invalid out stream. Must be either 'stdout' or valid directory.")
  elsif to != 'stdout'
    out = open(to + "/#{sample_id}.#{locus_str}.allele_counts.tsv", 'w')
...
def read_haplotype_file(path)
  ...
  haplotype_set = haplotype_file_lines.map{|sample_id,locus,coverage,freq,haplotype,metrics|
    {:sample_id => sample_id, :locus => locus, :coverage => coverage.to_i, :freq => freq.to_f, :haplotype => haplotype, :metrics => metrics}}
...
=end
  ...
  # then filter reads w/ haplotype shorter than specified min length
