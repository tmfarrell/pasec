...
workflow pasec {
	...
	# pool amplicon data together
	call pool_amplicons { 
		input: 
		raw_data_files = index_haplotypes.raw_data, 
		seq_index_files = index_haplotypes.seq_index, 
		filter_summary_files = index_haplotypes.filter_summary,
		haplotype_index_files = index_haplotypes.haplotype_index,
	...
	output { 
	   File run_unindexed = pool_amplicons.run_unindexed
       File seq_index = pool_amplicons.pooled_seq_index
       File filter_summary = pool_amplicons.pooled_filter_summary
       File haplotype_index_file = pool_amplicons.pooled_haplotype_index
       File read_metrics_file = pool_amplicons.read_metrics_file
	}	 	 
}
...
task index_haplotypes { 
	...
	command <<< 
		...
		first_non_empty_file=true 
		files_arr=(${sep=" " sample_haplotypes_files})
        for i in ${d}{!files_arr[@]} ; do 
        	if [ -s "${d}{files_arr[i]}" ] ; then 
        		if [ "${d}{first_non_empty_file}" = false ] ; then 
        			tail -q -n +2 ${d}{files_arr[i]} >> ${haplotypes_file}
        		else 
        			cat ${d}{files_arr[i]} > ${haplotypes_file}
        			first_non_empty_file=false
        		fi
        	fi 
        done
		# run 
        python ${analysis_script_path} \
          --run_id ${run_id} \
          --amplicon ${amplicon} \
          --metadata_file ${default='""' metadata_file} \
          --known_haplotypes_file ${default='""' known_haplotypes} \
          --min_population_freq ${default="0.000001" min_population_freq} \
          --run_haplotype_coverage_file ${haplotypes_file} \
          --output_dir . 
	>>> 

	output { 
		File raw_data = "${haplotypes_file}"
...
task pool_amplicons { 
	...
	command { 
		...
		# pool raw data files
		head -n 1 ${raw_data_files[0]} > run.unindexed.tsv
		tail -q -n +2 ${sep=" " raw_data_files} >> run.unindexed.tsv 
	}

	output { 
		File run_unindexed = "run.unindexed.tsv"
